...
Once you have obtained your API key and URL, set the following environment variables with the appropriate values:
  ...
  - `CORTEXCLOUD_API_KEY_TYPE` (set to `"standard"` or `"advanced"`)

## Request Tracking

The SDK automatically adds tracking headers to all API calls:

**X-Request-ID**: Unique identifier for each request (`req_<32-hex-chars>`). Customize via context:
```go
ctx = client.WithRequestID(ctx, "my-custom-id")
```

**User-Agent**: Versioned client identification (`cortex-cloud-go/1.0.0 (platform; go1.25.1; darwin/arm64)`). Customize via:
```go
client, err := platform.NewClient(config.WithAgent("my-app/1.0.0"))
```

**Logging**: Set `CORTEXCLOUD_LOG_LEVEL=debug` to see full request/response dumps with request IDs.
